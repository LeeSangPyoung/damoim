...
import { ProfileResponse, userAPI } from '../api/user';
...
const ProfileModal: React.FC<ProfileModalProps> = ({ profile, onClose, onSendMessage, onFriendStatusChange }) => {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      // 내 프로필 로드
      userAPI.getProfile(currentUser.userId)
        .then(setMyProfile)
        .catch(() => {});
    }
  }, [profile.userId]);
  ...
  const getSameClassInfo = () => {
    if (!myProfile || isMyProfile) return [];

    const sameClasses: Array<{
      schoolName: string;
    ...
    profile.schools.forEach(theirSchool => {
      myProfile.schools?.forEach(mySchool => {
        if (
          mySchool.schoolCode === theirSchool.schoolCode &&
          mySchool.graduationYear === theirSchool.graduationYear &&
          mySchool.grade === theirSchool.grade &&
          mySchool.classNumber === theirSchool.classNumber
        ) {
          sameClasses.push({
            schoolName: theirSchool.schoolName,
            graduationYear: theirSchool.graduationYear,
            grade: theirSchool.grade!,
            classNumber: theirSchool.classNumber!
          });
        }
      });
    });
  ...
  return (
    ...
    <div className="profile-modal-overlay" onClick={onClose}>
      <div className="profile-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        {/* 나와의 관계 */}
        {!isMyProfile && getSameClassInfo().length > 0 && (
          <div className="profile-modal-section">
            <div className="pm-relationship-card">
              <div className="pm-relationship-header">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/>
                  <path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
                </svg>
                <h3>우리는 같은 반 동창이에요!</h3>
              </div>
              <div className="pm-relationship-classes">
                {getSameClassInfo().map((info, idx) => (
                  <div key={idx} className="pm-same-class-item">
                    <div className="pm-same-class-icon">
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M20 6 9 17l-5-5"/>
                      </svg>
                    </div>
                    <div className="pm-same-class-info">
                      <div className="pm-same-class-school">{info.schoolName}</div>
                      <div className="pm-same-class-detail">
                        {info.graduationYear}년 졸업 · {info.grade}학년 {info.classNumber}반
                      </div>
                    </div>
                  </div>
                ))}
                      ...
                      <svg width="36" height="36" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <defs>
                          <linearGradient id="schoolGrad" x1="0" y1="0" x2="100" y2="100" gradientUnits="userSpaceOnUse">
                            <stop stopColor="#32373c" />
                            <stop offset="1" stopColor="#1a1d20" />
                          </linearGradient>
                        </defs>
                        ...
                        <rect x="26" y="50" width="14" height="12" rx="2" fill="#32373c" opacity="0.45" />
                        <rect x="60" y="50" width="14" height="12" rx="2" fill="#32373c" opacity="0.45" />
                        <rect x="42" y="56" width="16" height="26" rx="3" fill="#32373c" opacity="0.6" />
