...
export default function CreatePostModal({ onClose, onPosted, schoolName, graduationYear, activeTab }: CreatePostModalProps) {
  ...
  // 학년 변경 시 반 자동 선택하지 않음 (사용자가 직접 선택)
  ...
  const handleSubmit = async () => {
    ...
    // 우리 반일 때 학년/반 선택 확인
    if (visibility === 'CLASS' && (!selectedGrade || !selectedClassNumber)) {
      setModal({ type: 'error', message: '학년과 반을 선택해주세요.', onConfirm: () => setModal(null) });
      return;
    }
        ...
        <div className="create-post-footer">
          ...
          <div className="create-post-footer-right">
            ...
            <button
              ...
              disabled={posting || uploading || !content.trim() || (visibility === 'CLASS' && (!selectedGrade || !selectedClassNumber))}
            >
              {uploading ? (
