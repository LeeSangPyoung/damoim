...
const Board = () => {
  ...
  useEffect(() => {
    ...
    loadPosts();
  }, [activeTab, schoolName, graduationYear, filterGrade, filterClass]);
  ...
  const loadPosts = async () => {
    ...
    setLoading(true);
    try {
      const grade = activeTab === 'myClass' && filterGrade ? filterGrade : undefined;
      const cls = activeTab === 'myClass' && filterClass ? filterClass : undefined;
      const data = await postAPI.getPosts(user.userId, activeTab, schoolName, graduationYear, grade, cls);
      setPosts(data);
      if (data.length > 0) {
          ...
          <div className="board-filter-row">
            <div className="board-filter-chips">
              {gradeClasses.filter(gc => gc.classNumber).map((gc, i) => {
                const isActive = filterGrade === gc.grade && filterClass === gc.classNumber;
                return (
                  <button
                    key={i}
                    ...
                    onClick={() => {
                      if (isActive) {
                        ...
                        setFilterGrade(gc.grade);
                        setFilterClass(gc.classNumber || '');
                      }
                    }}
                  >
