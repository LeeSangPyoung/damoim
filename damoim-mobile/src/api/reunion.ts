...
export const reunionAPI = {
  createReunion: async (userId: string, data: { name: string; description?: string; schoolCode?: string; schoolName?: string; graduationYear?: string; coverImageUrl?: string; memberIds: string[] }): Promise<ReunionResponse> => {
  ...
  joinByCode: async (userId: string, inviteCode: string): Promise<ReunionResponse> => {
    const response = await apiClient.post('/reunions/join-by-code', null, { params: { userId, inviteCode } });
    return response.data;
  },

  getJoinRequests: async (reunionId: number, userId: string): Promise<JoinRequestResponse[]> => {
    const response = await apiClient.get(`/reunions/${reunionId}/join-requests`, { params: { userId } });
    return response.data;
  },

  approveJoinRequest: async (requestId: number, userId: string): Promise<void> => {
    await apiClient.put(`/reunions/join-requests/${requestId}/approve`, null, { params: { userId } });
  },

  rejectJoinRequest: async (requestId: number, userId: string): Promise<void> => {
    await apiClient.put(`/reunions/join-requests/${requestId}/reject`, null, { params: { userId } });
  },

  regenerateInviteCode: async (reunionId: number, userId: string): Promise<string> => {
    const response = await apiClient.put(`/reunions/${reunionId}/regenerate-code`, null, { params: { userId } });
    return response.data.inviteCode;
  },

  // 피드
  createPost: async (reunionId: number, userId: string, data: { content: string; imageUrls?: string[] }): Promise<ReunionPostResponse> => {
    const response = await apiClient.post(`/reunions/${reunionId}/posts`, data, { params: { userId } });
    return response.data;
  },

  getPosts: async (reunionId: number, userId: string): Promise<ReunionPostResponse[]> => {
    const response = await apiClient.get(`/reunions/${reunionId}/posts`, { params: { userId } });
    return response.data;
  },
  ...
  // 모임
  createMeeting: async (reunionId: number, userId: string, data: { title: string; description?: string; dateOptions: string[]; locationOptions: string[]; voteDeadline?: string }): Promise<MeetingResponse> => {
  ...
  confirmMeeting: async (meetingId: number, userId: string, finalDate: string, finalLocation: string): Promise<void> => {
    await apiClient.put(`/reunions/meetings/${meetingId}/confirm`, null, { params: { userId, finalDate, finalLocation } });
  },
  ...
  // 회비
  createFees: async (reunionId: number, userId: string, data: { amount: number; description?: string; dueDate?: string }): Promise<FeeResponse[]> => {
  ...
  updateFeePayment: async (feeId: number, userId: string, paidAmount: number): Promise<FeeResponse> => {
    const response = await apiClient.put(`/reunions/fees/${feeId}/pay`, null, { params: { userId, paidAmount } });
    return response.data;
  },

  getMeetingDetail: async (reunionId: number, meetingId: number, userId: string): Promise<MeetingResponse> => {
    const response = await apiClient.get(`/reunions/${reunionId}/meetings/${meetingId}`, { params: { userId } });
    return response.data;
  },
  ...
  addMemberToFeeGroup: async (feeGroupId: number, userId: string, targetUserId: string): Promise<FeeResponse> => {
    const response = await apiClient.post(`/reunions/fee-groups/${feeGroupId}/members`, null, { params: { userId, targetUserId } });
    return response.data;
  },
  ...
  assignTreasurer: async (reunionId: number, userId: string, targetUserId: string): Promise<void> => {
    await apiClient.put(`/reunions/${reunionId}/treasurer`, null, { params: { userId, targetUserId } });
  },
